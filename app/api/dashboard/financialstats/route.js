...
export async function GET(request) {
    ...
    const apiUrl = `${process.env.NEXT_PUBLIC_API_URL}/dashboard/financialstats${period ? `?period=${period}` : ""
        }`;
    ...
    if (!res.ok) {
        return new Response(JSON.stringify({ error: "Failed to fetch financial stats" }), {
            status: res.status,
        });
    }
